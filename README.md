...
- **Standardization:** Automatic conversion of Weight (Lbs → Kg) and dates.
...
```text
.
├── server/                 # BACKEND (Python ETL)
│   ├── main.py             # Entry point
│   ├── config.py           # User constants (Height/Age) & paths
│   ├── daily_brief.py      # CLI Reporting Tool
│   ├── requirements.txt    # Python dependencies
│   ├── data/               # YOUR RAW FITBIT DATA (Ignored by Git)
│   ├── modules/            # Logic (Parsers, Metrics, ETL)
│   └── plots/              # Static images output (Legacy V1)
│
...
│   │   └── ...
│   └── public/
│       └── dashboard_data.json # The feed generated by the server
│
└── dashboard-v1/           # Initial dashboard vanilla js version (Legacy V1)
```

## 🛠️ Prerequisites

1. **Python 3.13+** (for Data Processing)
2. **Node.js 24+** (for the Dashboard)
